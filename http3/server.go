...
func (s *Server) handleConn(conn *quic.Conn) error {
	...
	for {
		...
		go func() {
			...
			var prefetch bytes.Buffer
			frameType, err := quicvarint.Read(quicvarint.NewReader(io.TeeReader(str, &prefetch)))
			...
			hconn.handleRequestStreamWithPrefetch(str, prefetch.Bytes())
		}()
	}
	wg.Wait()
	return handleErr
